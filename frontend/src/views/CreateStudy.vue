...
const analysisHelp: Record<AnalysisType, { purpose: string; source: string; data: string; operations: string; result: string }> = {
  VARIANT_FREQUENCY: {
    purpose: 'Изчислява какъв дял от избраната многоцентрова целева група носи конкретен генетичен вариант.',
    source: 'Данните се взимат от локалната база на всяка болница: пациенти, възраст, пол, диагнози и връзките между пациенти и генетични варианти. Централният координатор не чете тези таблици.',
    data: 'Болницата първо избира пациентите, които отговарят на зададените критерии. Локално се изчисляват Nᵢ — броят съвпадащи пациенти, и Vᵢ — броят пациенти, които носят избрания вариант.',
    operations: 'Nᵢ и Vᵢ се разделят на Shamir дялове. BGW събира дяловете до глобални N и V, без да разкрива отделните болнични стойности. След разрешената реконструкция се изчислява V / N × 100.',
    result: 'Очакваният резултат съдържа общия размер на целевата група, общия брой носители и честотата на варианта в проценти.'
  },
  ALLELE_FREQUENCY: {
    purpose: 'Изчислява честотата на алтернативния алел за избрания вариант в общата целева група.',
    source: 'Използват се локалните таблици за пациенти, диагнози, генетични варианти и генотипа на всеки пациент за избрания вариант.',
    data: 'За всеки съвпадащ пациент генотипът се преобразува локално: 0/0 = 0, 0/1 или 1/0 = 1, а 1/1 = 2 алтернативни алела. Болницата получава Aᵢ — общ брой алтернативни алели, и Nᵢ — брой пациенти.',
    operations: 'Aᵢ и Nᵢ се споделят като Shamir дялове. BGW извършва защитено събиране до A и N. Едва след реконструкцията се пресмята A / (2 × N) × 100.',
    result: 'Показват се общата целева група, общият брой алтернативни алели и алелната честота в проценти.'
  },
  COHORT_MEAN_AGE: {
    purpose: 'Изчислява средната възраст на пациентите, които отговарят на избраните критерии.',
    source: 'Възрастта и критериите за подбор се четат единствено от локалните пациентски записи на всяка болница.',
    data: 'След локалното филтриране болницата пресмята Sᵢ — сумата на възрастите, и Nᵢ — броя на избраните пациенти. Индивидуални възрасти не се изпращат.',
    operations: 'Sᵢ и Nᵢ се превръщат в Shamir дялове и BGW ги сумира до глобални S и N. Деление не се изпълнява вътре в BGW; след разрешената реконструкция средната възраст е S / N.',
    result: 'Показват се общият брой пациенти и средната възраст на обединената целева група.'
  },
  THERAPY_RESPONSE_RATE: {
    purpose: 'Изчислява общия процент пациенти с положителен отговор към избраната терапия.',
    source: 'Данните идват от локалните записи за пациенти, приложени терапии, лечения и оценени терапевтични резултати.',
    data: 'Всяка болница брои Nᵢ — пациентите, лекувани с избраната терапия, и Rᵢ — тези от тях с резултат RESPONDER, след прилагане на критериите за целевата група.',
    operations: 'Rᵢ и Nᵢ се споделят чрез Shamir и се сумират защитено до R и N. След реконструкцията честотата се изчислява като R / N × 100.',
    result: 'Показват се терапията, общият брой лекувани пациенти, общият брой отговорили и процентът на отговор.'
  },
  THERAPY_RESPONSE: {
    purpose: 'Проверява асоциацията между носителството на вариант и отговора към избрана терапия.',
    source: 'Комбинират се локалните записи за пациенти, варианти и генотипи с леченията, терапиите и техните резултати.',
    data: 'Всяка болница изгражда локална 2×2 таблица: A — вариант и отговор, B — вариант без отговор, C — без вариант и отговор, D — без вариант и без отговор.',
    operations: 'Клетките се споделят и сумират защитено. BGW изчислява A×D и B×C чрез умножение и редукция на степента. След реконструкцията odds ratio е (A×D) / (B×C).',
    result: 'Показват се размерът на общата целева група и odds ratio. Локалните клетки A/B/C/D не се разкриват на координатора.'
  },
  VARIANT_DISEASE_ASSOCIATION: {
    purpose: 'Проверява дали носителството на избран вариант е асоциирано с наличието на избрано заболяване.',
    source: 'Използват се локалните пациентски записи, диагнозите и връзките към генетичните варианти и генотипи.',
    data: 'Всяка болница изгражда локална 2×2 таблица: A — вариант и заболяване, B — вариант без заболяване, C — без вариант и заболяване, D — без вариант и без заболяване.',
    operations: 'A/B/C/D се споделят и сумират чрез BGW. След това защитено се пресмятат A×D и B×C с редукция на степента, а след реконструкция — odds ratio = (A×D) / (B×C).',
    result: 'Показват се общият размер на целевата група и odds ratio, без автоматично медицинско заключение или разкриване на локалните стойности.'
  }
}
...
<template>
  <PageHead eyebrow="Нов защитен анализ" title="Създаване на проучване" description="Всяка болница определя целевата група локално." />
  <form class="grid max-w-4xl gap-4" @submit.prevent="submit">
      ...
      <div v-if="helpOpen" class="fixed inset-0 z-50 grid place-items-center bg-pine-950/55 p-4 backdrop-blur-sm"
        ...
        <section role="dialog" aria-modal="true" aria-labelledby="analysis-help-title"
          ...
          <div class="grid gap-5 p-6 text-left text-sm leading-relaxed">
            ...
            <aside class="rounded-xl border border-pine-100 bg-pine-50 p-4 text-xs text-pine-800"><b>Защита на данните:</b> индивидуалните пациентски записи и локалните агрегати остават в съответната болница. Между Hospital Nodes се обменят само Shamir дялове, а координаторът получава единствено изрично разрешения глобален резултат.</aside>
          </div>
        </section>
      </div>
    </Teleport>
    ...
    <section class="panel p-6">
      ...
      <p class="mt-1 text-xs text-mist">Изберете между 3 и 6 болници. Ролите P1–Pn се определят според реда на избиране.</p>
      <div v-if="form.organization_ids.length" class="mt-3 rounded-lg border border-line bg-paper px-4 py-3 text-xs">
        ...
        <span v-if="hasValidParticipantCount"> · Автоматичен праг: <b>t = {{ selectedThreshold }}</b></span>
      ...
      <label v-for="organization in eligible" :key="organization.id"
        ...
        <span class="min-w-0 grow text-left">
          ...
          <small class="mt-0.5 block text-xs text-mist">
            Hospital Node {{ organization.participant_index }}
